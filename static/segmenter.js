...
const NULL_RANK = 50_000;
const OOV_PENALTY = Math.log(NULL_RANK + 1);
...
// DP Segmentation
function segment(sentence, dict) {
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = Math.max(0, i - MAX_WORD_LEN); j < i; j++) {
      ...
      if (isPunct(word)) {
        ...
        oovAdd = 0;
        rankAdd = Math.log((dict[word].frequency ?? NULL_RANK) + 1);
      } else {
        oovAdd = word.length;
...
function classifyWord(word, dict, known) {
  ...
  return known.has(word) ? "known" : "unknown";
}
...
function analyzeSentence(sentence, dict, known) {
  const words = segment(sentence, dict).map((w) => ({
    word: w,
